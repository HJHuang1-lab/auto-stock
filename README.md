...
最上方設有 **5 個核心指數卡片**，展示近 1 個月的歷史日線收盤走勢：
1. **大盤加權指數 (`^TWII`)**：台股大盤指標。
2. **台指期貨近 (`WTX&`)** 🌙：台指近月期貨，涵蓋**日盤與夜盤**。透過爬取 Yahoo 奇摩股市即時報價，並以大盤歷史走勢等比縮放產生平滑的歷史趨勢線，每次排程自動更新。
3. **美元兌台幣匯率 (`USDTWD=X`)**：專業外匯指標，引進 **4 位小數高精度浮點格式** (如 `31.4140`)，滿足真實匯率波動細節。
4. **黃金期貨 (`GC=F`)**：避險商品指標。
5. **輕原油期貨 (`CL=F`)**：大宗原物料指標。
...
- **個股每日收盤價歷史走勢圖**：在個股詳細資訊面板中，於「公司營利與財務現況」卡片上方新增該個股近一個月（最多 60 天）的**每日收盤價歷史折線圖**。每天排程執行後自動追加最新一日收盤價，便於交易者直觀研判股價長期趨勢。滑鼠懸浮時顯示當日日期與收盤價。
...
- **自動化工作流步驟**：
  ...
  3. **資料更新與分析 (`data_exporter.py`)**：
     - 利用異步 ThreadPool 技術並行下載 133 支個股最新當日即時收盤報價，並追加更新至各股票歷史價格（`history`）記錄。
     - 抓取 TAIEX、台指期貨近 (WTX& 夜盤，來源：Yahoo 奇摩股市爬蟲)、美元兌台幣、黃金、原油近一個月日線趨勢數據。
...
│   │   ├── market_trends.json    # 大盤/台指期近(WTX&)/美金/黃金/原油一個月歷史走勢
│   │   ├── meta.json             # 儀表板最後更新時間等元數據
│   │   ├── optimization_weights.json  # 自我進化優化後的五大推薦權重
│   │   ├── stock_cache.json      # 133 支股票的即時報價、歷史每日收盤價與 AI 財務新聞雷達分析
